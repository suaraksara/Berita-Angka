...
halaman_detik = [str(i*40) for i in range(1, 21)]
...
for i in halaman_detik:
    indeks_cnbc = 'https://republika.co.id/index/'+ tanggal+'/'+i 
    sup_cnbc = bikin_sup(indeks_cnbc)
    berita_cnbc = sup_cnbc.find_all('div', class_='txt_subkanal')
    for a in berita_cnbc:
        b = a.find('h2')
        c = b.get_text()
        kabeh_judul.append(c)
        #print(b)
    for d in berita_cnbc:
        e = d.find('a')
        f = e['href']
        kabeh_link.append(f)
        #print(d)
...
berita_dg_angka.to_csv(namafile+'_'+'angka_dalam_republika.csv')
